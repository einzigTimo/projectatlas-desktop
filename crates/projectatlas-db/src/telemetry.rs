...
fn usage_events_for_project(
    ...
    let sql = if caller_label.is_some() {
        raw_event_select("AND i.caller_label = ?2", "ORDER BY e.id")
    } else {
        raw_event_select("", "ORDER BY e.id")
    };
...
/// Load the most recent retained raw usage events, newest first.
pub(crate) fn recent_usage_events(
    connection: &Connection,
    caller_label: Option<&str>,
    limit: u32,
) -> DbResult<Vec<UsageEvent>> {
    let project = current_project(connection)?;
    recent_usage_events_for_project(connection, project, caller_label, limit)
}

/// Load the most recent retained raw events for one captured project and optional label.
fn recent_usage_events_for_project(
    connection: &Connection,
    project: ProjectInstanceId,
    caller_label: Option<&str>,
    limit: u32,
    ...
    crate::project_identity::require_bound_project_identity(connection, project)?;
    let order_and_limit = if caller_label.is_some() {
        "ORDER BY e.created_at_epoch DESC, e.id DESC LIMIT ?3"
    } else {
        "ORDER BY e.created_at_epoch DESC, e.id DESC LIMIT ?2"
    };
    let sql = if caller_label.is_some() {
        raw_event_select("AND i.caller_label = ?2", order_and_limit)
    } else {
        raw_event_select("", order_and_limit)
    };
    let mut statement = connection.prepare(&sql)?;
    let mut rows = if let Some(label) = caller_label {
        statement.query(params![project.as_bytes().as_slice(), label, limit])?
    } else {
        statement.query(params![project.as_bytes().as_slice(), limit])?
    };
    let mut events = Vec::new();
    while let Some(row) = rows.next()? {
        events.push(map_usage_event(row)?);
...
fn raw_event_select(extra_predicate: &str, order_and_limit: &str) -> String {
    format!(
