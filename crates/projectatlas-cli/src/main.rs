            ...
            let report = run_init_bootstrap(
                ...
                Some(&|canonical_root: &Path| {
                    init_mcp_config_statuses(
                        &canonical_root.join(".projectatlas"),
                        &db_path,
                        &config_path,
                        false,
                    )
...
fn bind_project_root(
    ...
    let configuration_result: Result<(), CliError> = (|| {
        ...
        merge_project_claude_mcp_config(&root, &db_path, &config_path, nearest_project)?;
        Ok(())
    })();
...
///
/// Returns one status per generated config. The caller runs this before the
/// init scan so the project-scoped `.mcp.json` is indexed by the same run.
pub(crate) fn init_mcp_config_statuses(
    atlas_dir: &Path,
    db_path: &Path,
    config_path: &Path,
    nearest_project: bool,
    ...
    for (harness_name, file_name, harness) in [
        ...
        statuses.push(InitHostConfigStatus {
    ...
    if let Some(root) = atlas_dir.parent() {
        let path = root.join(PROJECT_CLAUDE_MCP_CONFIG_NAME);
        let (status, error) =
...
const PROJECT_CLAUDE_MCP_CONFIG_NAME: &str = ".mcp.json";

/// Merge the generated Claude Code server entry into `<root>/.mcp.json`.
///
/// Claude Code only auto-loads `.mcp.json` in the project root; the config
/// generated under `.projectatlas/` is invisible to it until copied there, which
/// silently leaves the server unregistered and token telemetry empty. Writing
/// the root file directly closes that gap. Entries for other MCP servers in an
/// existing `.mcp.json` are preserved; only the `projectatlas` entry is
/// inserted or replaced, and an already-current file is left untouched.
fn merge_project_claude_mcp_config(
    root: &Path,
    db_path: &Path,
    config_path: &Path,
    nearest_project: bool,
) -> Result<runtime::InitPhaseStatus, CliError> {
    let generated = build_harness_mcp_config_report(
        HarnessConfig::ClaudeCode,
        "projectatlas",
        db_path,
        Some(config_path),
    ...
    let generated_servers = generated
        ...
        .ok_or_else(|| {
            CliError::InvalidInput(
                "generated Claude Code MCP config is missing mcpServers".to_string(),
            )
        })?;
    let path = root.join(PROJECT_CLAUDE_MCP_CONFIG_NAME);
    let existed = path.exists();
    let mut document = if existed {
        ...
        serde_json::from_str::<serde_json::Value>(&text).map_err(|error| {
            CliError::InvalidInput(format!(
                "existing {} is not valid JSON ({error}); fix or remove it, then rerun init",
    ...
    let object = document.as_object_mut().ok_or_else(|| {
        CliError::InvalidInput(format!(
            "existing {} is not a JSON object; fix or remove it, then rerun init",
            normalize_native_path_display(&path)
        ))
    })?;
    let servers = object
    ...
    let servers = servers.as_object_mut().ok_or_else(|| {
        CliError::InvalidInput(format!(
            "mcpServers in existing {} is not a JSON object; fix or remove it, then rerun init",
            normalize_native_path_display(&path)
        ))
    })?;
    let mut changed = false;
    for (name, entry) in generated_servers {
