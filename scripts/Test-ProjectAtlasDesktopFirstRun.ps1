    ...
    try {
        ...
        if (-not $process.WaitForExit($timeoutMilliseconds)) {
            try {
                ...
                throw "Prozess ueberschritt das Zeitlimit von $ProcessTimeoutSeconds Sekunden und konnte nicht sicher beendet werden: $FilePath"
            }
            if (-not $process.WaitForExit(10000)) {
                throw "Prozess ueberschritt das Zeitlimit von $ProcessTimeoutSeconds Sekunden und blieb trotz Abbruch aktiv: $FilePath"
            }
            throw "Prozess ueberschritt das Zeitlimit von $ProcessTimeoutSeconds Sekunden und wurde fail-closed abgebrochen: $FilePath"
        }
